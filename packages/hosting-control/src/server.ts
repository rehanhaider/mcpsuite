 ...
 *   POST   /api/v1/workspaces                 — provision workspace + owner + defaults
...
import { sha256Hex, type Db } from "@emcp/db";
  ...
  async function runMutation(
    ...
    try {
      const execute = db.$client.transaction(() => {
        ...
        completeReceipt(db, idemKey, { targetHash: out.targetHash ?? null, httpStatus: out.status, responseBody });
        // The audit row commits atomically with the mutation and the receipt.
        writeAuditRow(rc, out.resultCode ?? "ok", out.status, false);
        return { status: out.status, responseBody };
      });
      const { status, responseBody } = execute();
      rc.audited = true;
    ...
    listen(): Promise<{ host: string; port: number }> {
      return new Promise((resolveListen, reject) => {
        ...
        server.listen(port, host, () => {
          ...
          resolveListen({ host, port: actualPort });
        });
      });
    },
    close(): Promise<void> {
...
function parseProvisionBody(body: Record<string, unknown>): ProvisionInput {
  ...
  const ownerEmail = optionalString(body, "ownerEmail", 320)?.trim();
  ...
  const ownerPassword = optionalString(body, "ownerPassword", 200);
  if (ownerPassword != null && ownerPassword.length < 10) {
    throw new HcError(400, "validation_error", "ownerPassword must be at least 10 characters");
  }
