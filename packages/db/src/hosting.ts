 ...
 * security context (a transaction can never switch workspaces). Receipts and
 * the service audit are hosting-control's own records and are not
 * workspace-bound: a receipt is read before any target is known. The outbox is
 * workspace-bound except for the sweep's list of pending deliveries.
 */
...
export interface HostingStore {
  ...
  // --- delivery outbox (workspace-bound; only the sweep's list spans workspaces)
  insertOutbox(row: { id: string; workspaceId: string; userId: string; purpose: AuthCodePurpose }): Promise<void>;
  markOutbox(workspaceId: string, id: string, state: "sent" | "abandoned" | "pending", lastError?: string): Promise<void>;
  /**
   * Every pending delivery, oldest first — the boot sweep's input and the one
   * read that spans workspaces. It returns hosting control's own delivery
   * references (ids and purpose), never CRM data.
   */
