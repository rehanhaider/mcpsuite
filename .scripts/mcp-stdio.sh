#!/usr/bin/env bash
# Launcher for the mcpsuite-crm MCP stdio server.
...
set -euo pipefail
cd "$(dirname "$0")/.."
# The stdio server authenticates with MCPSUITE_API_KEY (create a key in Admin ->
# Agents). If it isn't already in the environment, source it from an optional
# gitignored data/mcp.env (format: MCPSUITE_API_KEY=...). Proceeding without it is
# fine — the server itself prints a clear error and exits non-zero.
if [ -z "${MCPSUITE_API_KEY:-}" ] && [ -f data/mcp.env ]; then
  set -a
