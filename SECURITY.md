...
- The web app and MCP HTTP server bind to localhost by default and are meant
  ...
  — HTTP and stdio alike — must carry an mcpsuite API key (`Authorization: Bearer
  <key>` for HTTP, `MCPSUITE_API_KEY` for stdio); there is no anonymous mode.
...
- The SQLite database (`data/mcpsuite.db`) and its backups (`data/backups/`)
